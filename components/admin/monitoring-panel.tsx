...
import { ChevronDown, ChevronRight, LoaderCircle, Plus, RefreshCw, Trash2, X } from "lucide-react";

import { ConfirmDialog } from "@/components/ui/confirm-dialog";
import { Switch } from "@/components/ui/switch";
import type { Person, Repo, RepoDetail } from "@/lib/database";
...
export function MonitoringPanel() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    const [rRes, pRes] = await Promise.all([fetch("/api/admin/repos"), fetch("/api/admin/people")]);
  ...
  async function saveRoleMembers() {
    setRoleSaving(true);
    await fetch("/api/admin/repo-members", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ githubRepo: roleEdit.githubRepo, members: roleMembers }) });
    setRoleSaving(false);
  ...
  return (
    <div className="mx-auto max-w-5xl">
      <div className="mb-6">
        <h1 className="text-lg font-semibold tracking-tight">监控仓库</h1>
      ...
      <section className="mb-6 overflow-hidden rounded-lg border bg-card">
        <div className="flex items-center justify-between border-b px-5 py-3">
          <h2 className="text-sm font-semibold">仓库列表</h2>
        ...
        {syncMsg && <div className="border-b px-5 py-2 text-[11px] text-emerald-600 dark:text-emerald-500 bg-emerald-500/5">{syncMsg}</div>}
        <div className="overflow-x-auto">
          <table className="w-full border-collapse text-left">
            <thead><tr className="border-b bg-muted/30">
              ...
              <th className="h-8 w-16 px-3 text-[11px] font-medium text-muted-foreground last:pr-4">操作</th>
            </tr></thead>
            <tbody>
              {repos.length === 0 ? (
        ...
        <div className="grid gap-3">
          {roleMembers.map((m) => (
            <div key={m.githubId} className="flex items-center gap-2">
              ...
              <button type="button" onClick={() => setRoleMembers((prev) => prev.filter((x) => x.githubId !== m.githubId))} className="grid size-6 place-items-center rounded text-muted-foreground hover:text-destructive"><X className="size-3" /></button>
            </div>
          ))}
